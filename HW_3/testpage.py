...
class OperationsHelper(BasePage):
    ...
    def get_error_text(self):
        logging.info('Start get error text')
    ...
    def login_success(self):
        logging.info(f'Start get login success text')
        success_field = self.find_element(TestSearchLocators.LOCATOR_SUCCESS, time=2)
        text = success_field.text
        logging.info(f'We found text "{text}" in login field {TestSearchLocators.LOCATOR_SUCCESS[1]}')
        return text
    ...
    def find_new_post_title(self):
        logging.info('Start find new post title')
        time.sleep(1)
